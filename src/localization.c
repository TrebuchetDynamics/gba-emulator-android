...
static char* zh_localization_array[]={
    ...
    "Capture", "存档",
    "Restore", "读档",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "这个即时存档来自一个不兼容的构建. Garnacha Boy 已经尝试恢复, 但可能会有问题",
...
static char* hy_localization_array[]={
    ...
    "Capture", "Նկարեք",
    "Restore", "Վերա.",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Այս Պահվածկէ առաջացել է անհամատեղելի Վերսիաից: Garnacha Boy-ը կփորձի վերականգնել այն, բայց կարող են խնդիրներ լինել:",
...
static char* gr_localization_array[]={
    ...
    "Capture", "Αποθήκ.",
    "Restore", "Επαναφ.",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Αυτή η αποθηκευτική κατάσταση προέρχεται από μια μη συμβατή έκδοση. Το Garnacha Boy έχει προσπαθήσει να την ανακτήσει, αλλά μπορεί να υπάρχουν προβλήματα",
...
static char* nl_localization_array[]={
    ...
    "Capture", "Vastleggen",
    "Restore", "Herstellen",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Deze save staat komt van een oude versie. Garnacha Boy heeft geprobeerd hem te herstellen, maar er kunnen problemen zijn",
...
static char* da_localization_array[]={
    ...
    "Capture", "Gem",
    "Restore", "Gendan",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Dette snapshot kom fra en ikke-kompatibel build. Garnacha Boy har forsøgt at gendanne det, men der kan opstå fejl",
...
static char* fr_localization_array[]={
    ...
    "Capture", "Capturer",
    "Restore", "Restaurer",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Cet état de sauvegarde provient d'une version incompatible. Garnacha Boy a essayé de le récupérer, mais il peut y avoir des problèmes",
...
static char* de_localization_array[]={
    ...
    "Capture", "Erstellen",
    "Restore", "Laden",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Dieses Save stammt von einer inkompatiblen Version. Garnacha Boy hat versucht, es wiederherzustellen, aber es können Probleme auftreten.",
...
static char* it_localization_array[]={
  ...
  "Capture", "Cattura",
  "Restore", "Ripristina",
  "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Questo salvataggio di stato deriva da una build incompatibile. Garnacha Boy ha provato a recuperarlo, ma potrebbero esserci problemi",
...
static char* ru_localization_array[]={
  ...
  "Capture", "Поймать",
  "Restore", "Загрузить",
  "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Это состояние сохранения получено из несовместимой сборки. Garnacha Boy попытался восстановить его, но могут возникнуть проблемы",
...
static char* pl_localization_array[]={
    ...
    "Capture", "Zapisz",
    "Restore", "Wczytaj",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Zapis stanu pochodzi z niekompatybilnej kompilacji. Garnacha Boy próbował go przewrócić, ale mogą być problemy",
...
static char* sw_localization_array[]={
    ...
    "Capture", "Fånga",
    "Restore", "Återställ",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "Denna Sparnings Plats kom från en ostöd version. Garnacha Boy har försökt att återställa den, men det kan förekomma problem",
...
static char* ja_localization_array[]={
    ...
    "Capture", "保存",
    "Restore", "読込",
    "This save state came from an incompatible build. Garnacha Boy has attempted to recover it, but there may be issues", "このステートセーブは互換性のないビルドで作成されたものです。Garnacha Boy は復元を試みましたが、問題が発生する可能性があります",
